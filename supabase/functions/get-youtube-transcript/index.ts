...
async function fetchYouTubeTranscript(
  videoId: string,
): Promise<{ segments: any[]; fullText: string; error?: string } | null> {
  ...
  try {
    ...
    let selectedTrack = captionTracks.find(
      (t: any) => t.languageCode === "en" || t.languageCode?.startsWith("en") || t.vssId?.includes(".en"),
    );
    if (!selectedTrack) {
      selectedTrack = captionTracks[0];
    }
...
function getErrorMessage(errorCode: string): string {
  const messages: Record<string, string> = {
    CAPTION_DOWNLOAD_DISABLED:
      "Caption downloads are disabled for this video by the owner. The video owner has restricted access to captions. Please upload the video file directly instead.",
    NO_CAPTIONS:
      "This video does not have captions available. Enable auto-generated captions in YouTube Studio or upload the video file directly.",
    ...
    API_FORBIDDEN:
      "Cannot access this video's captions. The video may be private or have restricted access. Try uploading the video file directly.",
  };
