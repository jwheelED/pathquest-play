...
serve(async (req) => {
  ...
  try {
    ...
    const { data: isInstructor, error: roleError } = await supabaseClient.rpc("has_role", {
      _user_id: user.id,
      _role: "instructor",
    });

    if (roleError || !isInstructor) {
      return new Response(JSON.stringify({ error: "Instructor access required" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (transcript.length < 30) {
      return new Response(JSON.stringify({ error: "Transcript too short (minimum 30 characters)" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (transcript.length > 50000) {
      return new Response(JSON.stringify({ error: "Transcript too long (maximum 50,000 characters)" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (/[\x00-\x08\x0B\x0C\x0E-\x1F]/.test(transcript)) {
      return new Response(JSON.stringify({ error: "Transcript contains invalid characters" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (materialContext !== undefined && materialContext !== null) {
      if (!Array.isArray(materialContext)) {
        return new Response(JSON.stringify({ error: "materialContext must be an array" }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      ...
      if (materialContext.length > 10) {
        return new Response(JSON.stringify({ error: "materialContext cannot contain more than 10 items" }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      ...
      for (let i = 0; i < materialContext.length; i++) {
        ...
        if (typeof material.content === "string" && material.content.length > 10000) {
          return new Response(JSON.stringify({ error: `Material ${i + 1} content exceeds 10,000 characters` }), {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
...
Question format:
{
  ...
  "expectedAnswer": "A" | "B" | "C" | "D" // for multiple choice, provide ONLY the letter
    ...
    if (!response.ok) {
      ...
      return new Response(JSON.stringify({ error: "Failed to generate questions. Please try again." }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: "Invalid response format. Please try again." }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
