...
| EV-011 | Load/reconnect/failure harness | P0 | **Partial** | `scripts/simulate-classroom.ts` now runs 25/100/200 cohorts × N repeated questions and reconciles expected (N×Q) vs actual rows incl. **duplicate detection**; **still no reconnect/late-join/dup-tap/AI-failure scenarios; team-run (not a CI job); not yet run** | Not Run |
